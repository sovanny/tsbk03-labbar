...
Model *model1;
FBOstruct *fbo1, *fbo2;
GLuint phongshader = 0, plaintextureshader = 0, lowpassshader = 0, truncshader = 0, addshader = 0;
...
void display(void)
{
	...
	// render to fbo1!
	// nu ritar vi inte till skärmen, utan till fbo1
	useFBO(fbo1, 0L, 0L);
	...
	//lowpass en gång
	runfilter(lowpassshader,fbo2,0L,fbo2);

	//pingponga lowpass. lagg!!!
	for(int i = 0; i < 10; i++){
		runfilter(lowpassshader, fbo2, 0L, fbo2);
	}
